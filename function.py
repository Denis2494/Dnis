...
from Famzbotz import TalkService
from Famzbotz.ttypes import *
import time, json, livejson, requests, threading, os, random, ast, datetime, sys, re, shutil

class Famz_botz:
    def __init__(self, myToken, myApp, pool=False):
        self.lineServer =  "https://legy-jp-addr-long.line.naver.jp"
        self.thisHeaders = {}
        splited = myApp.split("\t")
        ...
        elif splited[0] in ["IOS", "IOSIPAD", "ANDROID"]:
    ...
    def downloadMsg(self, msgid, name= ".bin"):
        path = '{}'.format(name)
        r = requests.session().get( 'https://obs-sg.line-apps.com/talk/m/download.nhn?oid='+msgid, headers=self.thisHeaders, stream=True)
